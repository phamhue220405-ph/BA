...
const ShippingModule = {
  ...
  create(data) {
    const { name, estimatedDays, regions, costs, status, description } = data;
    if (!name || !estimatedDays) return { error: 'Thiếu thông tin bắt buộc' };
    const method = StorageService.save('crb_shipping_methods', {
      name, estimatedDays: Number(estimatedDays),
